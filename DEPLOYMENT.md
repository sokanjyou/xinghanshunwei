...
接口固定使用免密搜索，不读取或使用 `TAVILY_API_KEY`、`WEB_SEARCH_API_KEY`。Tavily Keyless 只适合试用和低流量场景，公共额度触发限流时会尝试免密后备搜索。

新闻和体育查询在 Tavily Keyless 不可用时会自动改用 Google News RSS 免密检索。所有免密检索都失败时，接口会返回面向用户的保守说明，不猜测比分、价格或实时结果，也不会暴露技术错误。
